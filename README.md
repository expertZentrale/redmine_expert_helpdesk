...
![Ticket page with the helpdesk info bar: an SLA chip for the reaction time (met) and one
for the solution time (due), the sender's name and address, and the mail's files — a
signature logo, two icons and the customer's screenshot, each with a Block
button](docs/screenshots/en/04-issue-detail.png)
...
Replies are written in the normal Redmine note field. While the reply is switched on, the
recipients, the note and the preview of the signature that will be appended form one
block with a hazard border, so it cannot be mistaken for an internal note. Whoever the
customer had in To or Cc is offered next to the fields.

![The customer reply armed in the ticket edit form: one block framed by a diagonal hazard
border, holding the recipient, To/CC/BCC fields with an "original recipients" offer, the
ticket's files to attach, the note, and the signature preview after
it](docs/screenshots/en/06-reply.png)

### AI answer draft

With the knowledge base enabled, the note toolbar drafts the reply from similar resolved
tickets and names the tickets it is based on.

![An AI answer draft in the armed customer reply: numbered steps for a scanner that
stopped being recognised, a "Based on" line citing two resolved tickets with their
relevance, and a warning that the text was generated by AI and must be checked before
sending](docs/screenshots/en/12-ai-answer-draft.png)

### Blocking signature logos

Signature logos and icons can be blocked once per project instead of deleted on every
ticket; the match is the file content, not its name.

![A ticket's files: a signature logo, two social-media icons and the customer's actual
screenshot, each row with a Block button](docs/screenshots/en/13-attachment-block.png)
