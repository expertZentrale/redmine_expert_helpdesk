...
- **`knowledge_retrieval.rb`** — the one RAG search path, shared by the summary job and the
  answer drafter. `KnowledgeRetrieval.search` holds the settings defaults (`kb_top_k`,
  `kb_min_score`, `kb_min_results`), the self-hit rejection and the all-or-nothing `min_results`
  rule; the *caller* keeps the gate that differs (summary: `kb_show_in_*?`, drafter: its own
  threshold). `format_hits(with_issue_ids:)` is load-bearing, not cosmetic — the summary names
...
- **`answer_drafter.rb`** — customer-facing answer drafts ("KI-Antwortvorschlag"), the only AI
  ...
  waiting), so it carries its own timeout (`ai_answer_timeout`, default 20 s, well under any
  reverse proxy), its own token budget (`ai_answer_max_tokens`) and a per-user/per-issue throttle.
  Both are applied by constructing `AiClient.new(settings.merge(...))` — the client reads its
  limits from the hash it is built with, so no other call site is affected.
  **Input is narrower than the extractor's on purpose**: subject, description and *public* notes
  only (`KnowledgeExtractor.ticket_text(include_private: false)`), no attachments, and replies
  that came from a draft are excluded (`HelpdeskMessage#ai_drafted`) so the model never re-ingests
  its own output. **No knowledge-base hit means no draft** — the three variants that propose a fix
  refuse (`NoGroundingError`), only `ask` (Rückfrage) runs ungrounded because it proposes nothing.
  `ai_answer_min_score` (central + per-project, default 0.65) is deliberately separate from
  `kb_min_score`: one governs text to customers, the other proposals to agents. The mailbox's
  reply header/footer are rendered into the prompt so the customer never gets two salutations.
  Off by default, opt-in per project; logged as `answer_draft` in `helpdesk_ai_requests` including
  the acting `user_id`. Migrations 052–055.
...
- **`legacy_contacts_import.rb`** — one-off import from the `redmine_contacts` plugin, plus
  ...
  (file store is per pod). One live run at a time; no heartbeat for `STALE_AFTER` (1 h) = dead,
  since the `:async` adapter loses queued jobs on restart.
