...
A third gem, @pg@, is needed only by the optional pgvector backend of the knowledge base; it is deliberately not in @PluginGemfile@, so the default Qdrant build does not drag in libpq. Both backends are off unless you enable them.
...
| Manage helpdesk mailboxes | Mailbox configuration, folders, rules, answer templates, blocked attachments |
...
| View knowledge base | The project's "Knowledge base" tab: list and detail of the RAG entries |
| Edit knowledge base entries | Correct, create, approve and reject entries (re-embeds on save); also the knowledge-base buttons in the ticket sidebar |
| Manage knowledge base | Delete entries and rebuild the project's vector index |
...
| Reply block | The two colours of the customer-facing block in the ticket form: its background and the warning border shown while a reply is armed. Hex colours, each overridable per project; a colour picker and a live sample sit next to the fields |
| Blocked attachments | The two guards on the *Block* button: the file types it may be offered for (default @png, gif, jpg, jpeg, bmp, webp, tif, tiff, ico@; @*@ allows all) and the largest file it may be offered for (default 100 KB, @0@ disables the limit). Both can be overridden per project; they gate the button only and never drop anything from incoming mail |
...
| AI answer drafts | Master switch for the customer-facing draft button, plus its prompt, minimum match, output budget and timeout. Off by default, and each project opts in separately. Requires the AI settings, and the knowledge base for every variant except "ask for missing details" |
| Knowledge base reranker | Optional second retrieval stage: a cross-encoder re-scores the shortlist the vector store returns, which is markedly more precise than similarity alone. Off by default; endpoint and key fall back to the embeddings ones, since both models usually share a base URL. Its own minimum score replaces the similarity one while it is on, and both want retuning — the two are different scales |
